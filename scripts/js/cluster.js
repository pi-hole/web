...
function members(data) {
  ...
  for (const peer of data.cluster.peers ?? []) {
    ...
    list.push({
      ...
      // A node too old to publish these says nothing, and said nothing before
      // the fields existed either - so absent reads as healthy
      resolving: peer.resolving !== false,
      vipCapable: peer.vip_capable !== false,
      identityShared: peer.identity_shared === true,
  ...
  if (!seen.has(node.id) && list.every(entry => !entry.self)) {
    list.unshift({
      ...
      resolving: node.resolving !== false,
      vipCapable: node.vip_capable !== false,
  ...
  // Whether a second machine carries this node's identity is something only
  // this node knows about itself, and it is published beside the rest of its
  // health rather than per peer - so it is attached to whichever entry turned
  // out to be this one, however that entry got into the list
  // This node knows it about itself; every other member publishes it too, so a
  // node standing down for a shared identity does not read as a healthy member
  // with nothing to do on anybody else's page
  for (const entry of list) {
    if (entry.self) {
      entry.identityShared = node.identity_shared === true;
    }
  }
...
function renderNodes(list, data) {
  ...
  for (const entry of list) {
    // Reachable is not the same as working: a node whose dnsmasq did not start
    // answers this page perfectly well and answers no queries at all, and the
    // cluster already declines to give it the address or DHCP for that reason
    const sick = entry.reachable && entry.resolving === false;
    // A node that shares its identity with another machine takes no part in
    // DHCP or the virtual address, and looks from here exactly like one that
    // simply has nothing to do
    const twinned = entry.reachable && entry.identityShared === true;
    const state = !entry.reachable ? "danger" : sick || twinned ? "warning" : "success";
    const detail = !entry.reachable
      ? utils.escapeHtml(entry.error ?? "unreachable")
      : sick
        ? "answers no DNS - its resolver did not start"
        : twinned
          ? "another machine is using this node's identity - serving nothing until one of them is given a new one"
          : entry.vipCapable === false
            ? "cannot hold the virtual address"
            : "in the cluster";
...
function renderTopology(list, data) {
  ...
  const view = (a, b) => {
    ...
    if (!a.entry.reachable || !Array.isArray(a.entry.sees) || !b.entry.url) {
      return null;
    }

    // By address: an identity is forgotten a few rounds after a node stops
    // answering, and matching on it drew a link between any two members whose
    // identities this node had dropped - on the page whose only job is to show
    // which links are down
    return a.entry.sees.includes(b.entry.url);
  };
  ...
  // Present tense only where somebody is actually handing out leases. The owner
  // is also the node that *would* take DHCP over, and saying that node serves it
  // when it never has is the page telling the operator something untrue about
  // the one thing they came to check
  const serving = list.some(entry => entry.dhcp && entry.dhcp.active);
  if (owner) {
    notes.push(
      serving
        ? `DHCP is served by <strong>${utils.escapeHtml(owner)}</strong>.`
        : `No node is handing out leases; <strong>${utils.escapeHtml(
            owner
          )}</strong> would take DHCP over.`
    );
  }
  ...
  // The owner is named whenever somebody hands out leases, failover or not.
  // Whether DHCP moves at all is what the warning is about, and that is a
  // per-node switch every member publishes
  const moving = list.some(entry => entry.dhcp && entry.dhcp.failover === true);
  if (owner && !vip && moving) {
    notes.push(
    ...
    // Why it is nowhere decides what to say. Sending somebody to check an
    // interface setting when no node ever tried to place the address - because
    // clustering is not running, or because none of them can hold it - is
    // pointing at the wrong thing
    const running = data.cluster.running !== false;
    const anyCapable = list.some(entry => entry.reachable && entry.vipCapable !== false);

    // A node this one cannot poll reports nothing about the address, so "on no
    // node" is a claim this page is not in a position to make. Saying it anyway
    // sent the operator to check an interface setting on a cluster whose
    // address was placed and working, on a machine they could not see
    const unseen = list.filter(entry => !entry.reachable);
    if (unseen.length > 0 && running) {
      notes.push(
        `<span class="text-body-secondary"><strong>${utils.escapeHtml(vip)}</strong> is on no node ` +
          `this one can reach - ${utils.escapeHtml(
            unseen.map(entry => entry.name).join(", ")
          )} ${unseen.length === 1 ? "does" : "do"} not answer, and may be holding it.</span>`
      );
    } else {
      const why = !running
        ? "clustering is not running here, so nothing has tried to place it"
        : !anyCapable
          ? "no node in the cluster can hold it - see the nodes above"
          : "check <code>cluster.vip.interface</code>, and the log for why it could not be placed";
      notes.push(
        `<span class="text-warning"><i class="fa-solid fa-triangle-exclamation"></i> <strong>${utils.escapeHtml(
          vip
        )}</strong> is on no node - ${why}.</span>`
      );
    }
  }
...
function renderPeers(list) {
  ...
  for (const entry of list) {
    ...
    const BASELINE_MAX = 1_000_000_000;
...
async function takenAddresses() {
  ...
  for (const iface of interfaces?.interfaces ?? []) {
    const global = (iface.addresses ?? []).filter(
      address => address.family === "inet" && address.scope === "universe"
    );
    for (const address of global) {
...
function setupJoin() {
  ...
  go.addEventListener("click", () => {
    ...
    // A node with a second factor answers a password on its own with no
    // session at all, and the refusal cannot say which of the two was missing
    const totp = document.querySelector("#join-totp").value.trim();
    if (totp !== "") {
      body.totp = totp;
    }
    ...
    // Leaving a cluster asks, and it loses nothing. Joining one replaces this
    // Pi-hole's groups, clients, domains and adlists with the cluster's, and
    // there is nothing to undo it with
    if (
      !confirm(
        `Join ${body.url}?\n\nThis Pi-hole's own groups, clients, domains, adlists, settings and web password are replaced by that cluster's. This cannot be undone.`
      )
    ) {
      return;
    }
...
function refresh() {
  return Promise.all([api("/cluster/status"), api("/config")]).then(([status, config]) => {
    ...
    if (enabled && !running) {
      const configured = config.config.cluster.members ?? [];
